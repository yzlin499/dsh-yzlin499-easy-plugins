    ...
    function apply(ctx) {
      ...
      function SettingsCard() {
        ...
        const T = lang === "en" ? {
          ...
          grantLabel: "Auto-allow a call when a rule hits or AI allows",
          grantHelp: "This mode runs at danger-full-access and every call is audited before it executes. When on, read-only, in-workspace, allowlisted, and AI-allowed calls run automatically; everything else goes to interactive approval. When off, every call goes to interactive approval. Mass-destructive operations always go to a human.",
          ...
          grantLabel: "命中规则或 AI 通过时，自动放行该次调用",
          grantHelp: "该模式在完整环境（danger-full-access）下运行，每个工具调用执行前都会被审计。开启时，只读、工作区内、白名单命中与 AI 通过的调用自动放行，其余转人工审批；关闭后所有调用都转人工确认。批量破坏类操作永远转人工。",
